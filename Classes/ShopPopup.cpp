...
void ShopPopup::createConfirmButton()
{
    auto parent = this->getParent();
    if (!parent) return;
    ...
    confirmButton->addTouchEventListener([this](Ref* sender, ui::Widget::TouchEventType type) {
        if (type == ui::Widget::TouchEventType::ENDED) {
            this->confirmArchPlacement();
        }
        });

    parent->addChild(confirmButton, 1000);
}
...
void ShopPopup::removeCancelAndConfirmButtons()
{
    auto parent = this->getParent();
    if (parent) {
