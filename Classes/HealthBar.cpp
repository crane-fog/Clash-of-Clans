...
bool HealthBar::init(float max_hp, float max_width, bool is_troop)
{
    ...
    this->setContentSize(Size(max_width_, max_height_));

    // 创建背景（黑色半透明）
    bar_bg_ = ui::Scale9Sprite::create("LoadingBarFile.png");

    bar_bg_->setContentSize(Size(max_width_, max_height_));
    bar_bg_->setColor(Color3B(0, 0, 0)); // 黑色
    bar_bg_->setOpacity(128); // 半透明
    bar_bg_->setAnchorPoint(Vec2(0.5f, 0.5f));
    bar_bg_->setPosition(Vec2(max_width_ / 2, max_height_ / 2)); // 相对于 HealthBar 自身
    this->addChild(bar_bg_, 10); // zOrder 较高确保在上层

    // 创建前景（从左到右填充，颜色根据血量变化）
    bar_fg_ = ui::Scale9Sprite::create("LoadingBarFile.png");
    bar_fg_->setContentSize(Size(max_width_ - 2 * padding_, max_height_ - 2 * padding_));
    if (is_troop_) bar_fg_->setColor(Color3B::GREEN); // 士兵血条初始为绿色
    else bar_fg_->setColor(Color3B(216, 73, 197));//建筑血条初始为紫色
    bar_fg_->setAnchorPoint(Vec2(0.0f, 0.5f));// 左中对齐
    bar_fg_->setPosition(Vec2(padding_, max_height_ / 2.0f)); // 相对于 HealthBar 自身
...
void HealthBar::hideBar(float dt)
{
    this->setVisible(false);
}
